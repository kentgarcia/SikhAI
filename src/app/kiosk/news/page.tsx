...
export default function KioskNewsPage() {
    ...
    return (
        <div className={`min-h-screen flex flex-col ${highContrast ? 'bg-black text-white' : 'bg-white text-black'} ${largeText ? 'text-2xl' : ''}`}
            aria-label={lang === 'tl' ? 'Balita' : 'News'} lang={lang === 'tl' ? 'tl' : 'en'}>
            ...
            <header role="banner" className={`flex items-center gap-4 px-8 py-6 border-b ${highContrast ? 'bg-black border-gray-700' : 'bg-white/80 border-gray-200'}`}>
                ...
                <h1 className={`ml-4 font-bold tracking-tight ${largeText ? 'text-4xl' : 'text-2xl'} ${highContrast ? 'text-yellow-400' : 'text-primary'}`}>{lang === 'tl' ? 'Balita' : 'News'}</h1>
            </header>
                    ...
                    {news.map(item => (
                        ...
                        >
                            ...
                            <div className="flex-1 p-6 flex flex-col gap-2 justify-center">
                                <h2 className={`font-bold ${largeText ? 'text-2xl' : 'text-xl'} ${highContrast ? 'text-yellow-400' : 'text-primary'}`}>{item.title}</h2>
                                <div className="flex items-center text-sm mb-2">
                                    <Calendar className="w-4 h-4 mr-2" aria-hidden="true" />
                                    <span className={highContrast ? 'text-yellow-200' : 'text-gray-600'}>{item.date}</span>
                                </div>
                                <p className={`${largeText ? 'text-lg' : 'text-base'} ${highContrast ? 'text-yellow-200' : 'text-gray-700'}`}>{item.description}</p>
                            </div>
                        </article>
                    ))}
                </div>
